	...
	public EntityModel<ConsoleGame> toModel(final ConsoleGame consoleGame) {
		return EntityModel.of(consoleGame,
				...
				linkTo(methodOn(AgeCategoryController.class).findById(consoleGame.getAgeCategory().getId()))
						.withRel("ageCategory"),
				linkTo(methodOn(GameplayModeController.class).findById(consoleGame.getGameplayMode().getId()))
						.withRel("gameplayMode"),
				linkTo(methodOn(GenreController.class).findById(consoleGame.getGenre().getId())).withRel("genre"),
				linkTo(methodOn(HardwarePlatformController.class).findById(consoleGame.getHardwarePlatform().getId()))
						.withRel("hardwarePlatform"),
				linkTo(methodOn(LanguageController.class).findById(consoleGame.getLanguage().getId()))
						.withRel("language"),
				linkTo(methodOn(ProducerController.class).findById(consoleGame.getProducer().getId()))
						.withRel("producer"));
	}

	@Override
	public CollectionModel<EntityModel<ConsoleGame>> toCollectionModel(
